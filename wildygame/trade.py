...
class TradeManager:
    """
    Helper manager used by Wilderness cog.

    The cog must provide:
      - bot
      - _mem_lock (asyncio.Lock)
      - _get_player(member/user)
      - _resolve_item(query), _resolve_food(query), _resolve_from_keys_case_insensitive(query, keys), _norm(str)
    ...
    async def _safe_edit(self, interaction: discord.Interaction, *, embed: discord.Embed, view: Optional[discord.ui.View]):
        """
        Safely edits the message that the button interaction came from.
        """
        try:
            if interaction.response.is_done():
